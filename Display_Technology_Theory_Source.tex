...
% Add remaining chapters with same detailed treatment...
% Due to length, showing structure for remaining parts:

\chapter{HDR Transfer Functions}
\section{Perceptual Quantizer (PQ / ST.2084)}
% Detailed PQ formulas, graphs, examples

\section{Hybrid Log-Gamma (HLG / BT.2100)}
% Detailed HLG formulas, comparison with PQ
...
\begin{itemize}
\item Resolution: 1500+ PPI per eye (retinal)
\item Refresh rate: 90+ Hz (120+ preferred)
\item Persistence: <3ms (avoid motion blur)
\item Latency: <20ms (motion-to-photon)
\item Field of view: 90--110 degrees
\item IPD adjustment: 58--72mm range
\end{itemize}

\chapter{Quantum Dot Technology}
\section{Working Principle}
Quantum dots (QDs) are semiconductor nanocrystals with size-tunable emission:
\begin{equation}
\lambda_{\text{emission}} \propto \frac{1}{r^2}
\end{equation}
where $r$ is the QD radius (2--10 nm).
...
\begin{tikzpicture}[scale=0.8]
  ...
  \begin{scope}
    \foreach \x/\col in {0/0.0, 1/0.3, 2/0.7, 3/1.0, 4/0.7, 5/0.3, 6/0.0} {
      \fill[black!\col*100] (\x,0) rectangle +(1,1);
  ...
  \begin{scope}[yshift=-2.5cm]
    \foreach \x/\r/\g/\b in {
      ...
      \fill[red!\r*100] (\x*0.333,0) rectangle +(0.333,1);
      \fill[green!\g*100] (\x*0.333+0.333,0) rectangle +(0.333,1);
      \fill[blue!\b*100] (\x*0.333+0.666,0) rectangle +(0.333,1);
